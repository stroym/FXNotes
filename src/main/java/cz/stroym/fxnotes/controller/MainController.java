...
public class MainController {
  ...
  private void initialize() {
    ...
    FILE_CHOOSER.getExtensionFilters()
                .add(TEXT_FILTER);
    notesView.setItems(notebook.getDefaultSection()
                               .getObservableNotes());
  ...
  private void setupDragAndDrop() {
    ...
    tagsView.setOnDragDetected(event -> {
      ...
      content.putString(tagsView.getSelectionModel()
                                .getSelectedItem()
                                .getValue());
  ...
  private void addNote() {
    ...
    notesView.getSelectionModel()
             .selectLast();
    notesView.edit(notesView.getSelectionModel()
                            .getSelectedIndex());
    notesView.scrollTo(notesView.getSelectionModel()
                                .getSelectedIndex());
  }
  ...
  private void deleteNote() {
    getSelectedSection().getNotes()
                        .remove(getSelectedNote());
  }
  ...
  private void addTag() {
    ...
    tagsView.layout();
    tagsView.getSelectionModel()
            .selectLast();
    tagsView.edit(tagsView.getSelectionModel()
                          .getSelectedIndex());
    tagsView.scrollTo(tagsView.getSelectionModel()
                              .getSelectedIndex());
  }
  
  @FXML
  private void showAll() {
    sectionsView.getSelectionModel()
                .select(-1);
    notesView.getSelectionModel()
             .select(-1);
    noteTextArea.setText("");
  ...
  private void handleExport(File file) {
    try {
      if (notebook.getSections()
                  .isEmpty()) {
        DialogUtils.generateInfo("Information", "Nothing to save", null)
                   .showAndWait();
      }
      
      DataUtils.writeJson(file, notebook);
    } catch (IOException e) {
      DialogUtils.generateError("An error has occurred!", "Couldn't write to file", e)
                 .showAndWait();
    }
  }
